...
const calendar = {
  ...
  solar2lunar: function (y, m, d) { // 参数区间1900.1.31~2100.12.31
    ...
    // 修正ymd参数
    let _y = objDate.getFullYear()
    let _m = objDate.getMonth() + 1
    let _d = objDate.getDate()
    ...
    let isToday = false
    if (isTodayObj.getFullYear() == y && isTodayObj.getMonth() + 1 == _m && isTodayObj.getDate() == _d) {
      isToday = true
    }
    // 星期几
    ...
    let firstNode = this.getTerm(_y, (_m * 2 - 1))// 返回当月「节」为几日开始
    let secondNode = this.getTerm(_y, (_m * 2))// 返回当月「节」为几日开始
    ...
    let dayCyclical = Date.UTC(_y, sm, 1, 0, 0, 0, 0) / 86400000 + 25567 + 10
    let gzD = this.toGanZhi(dayCyclical + _d - 1)
    // 该日期所属的星座
    let astro = this.toAstro(_m, _d)

    return { 'lYear': year, 'lMonth': month, 'lDay': day, 'Animal': this.getAnimal(year), 'IMonthCn': (isLeap ? '\u95f0' : '') + this.toChinaMonth(month), 'IDayCn': this.toChinaDay(day), 'cYear': _y, 'cMonth': _m, 'cDay': _d, 'gzYear': gzY, 'gzMonth': gzM, 'gzDay': gzD, 'isToday': isToday, 'isLeap': isLeap, 'nWeek': nWeek, 'ncWeek': '\u661f\u671f' + cWeek, 'isTerm': isTerm, 'Term': Term, 'astro': astro }
  },
}
